...
def update_sim(pubsocket, time_stamp):
    ...
    striker = gPlayers[me.myPlayer].groups[me.strikeTeam]
    if striker.moving is False and striker.location != me.targetNode:
        #if gMap.nodes[group.location - 1].controlledBy == me.myPlayer:
        possibleConnections = []
        foundTarget = None
        destination = None
        for connection in gMap.nodes[striker.location - 1].connections:
        ...
        evgcom.PY_GROUP_MoveToNode(pubsocket, me.guid, striker.groupID, destination)
        print("send group move for {0}".format(striker.groupID))
        striker.moving = True
    if me.inPlace == False:
        me.inPlace = True
        i = 0
        for connection in gMap.nodes[me.startNode - 1].connections:
            if i == me.strikeTeam:
                i = i + 1
            if i < len(gPlayers[me.myPlayer].groups):
                evgcom.PY_GROUP_MoveToNode(pubsocket, me.guid, gPlayers[me.myPlayer].groups[i].groupID, connection.destID)
            i = i + 1
...
def program_main(): 
    ...
    evgcom.HelloWorld(pubsocket, guidstrP0, "stallAndSneak")
    waiting = True
    while waiting:
        # Read envelope with address
        try:
