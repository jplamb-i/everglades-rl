...
class Everglades(gym.Env):
    ...
    def reset(self):
        ...
        while len(self.units) < self.num_units:
            ...
            _counter += 1
            if _counter > 50:
                logger.warning('Still awaiting receipt of group initialization. Attempt {}'.format(_counter))
            sleep(.1)

        logger.info('Received group initialization receipts ({} Units).'.format(len(self.units)))
        ...
        while not waiting_for_action:
            ...
            _counter += 1
            if _counter > 50:
                logger.warning('Still waiting for turn to start. Attempt {}'.format(_counter))
            sleep(.1)
