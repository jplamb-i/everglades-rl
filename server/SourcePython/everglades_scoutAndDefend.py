...
class me:
    ...
    topLaneThreat = False
    botLaneThreat = False
    homeBaseThreat = False

    #p0 paths
    atkPathTop = [2, 5, 8, 11]
    atkPathBot = [4, 7, 10, 11]
...
def init_data(guid, mapFilePath, unitFilePath, playerFilePath, player):
    if me.opPlayer == player:
        ...
        me.defendNodeTop = 8
        me.defendNodeBot = 10
        me.atkPathTop = [8, 5, 2, 1]
        me.atkPathBot = [10, 7, 4, 1]
        me.scoutNodeBot = 4
        me.scoutNodeTop = 2
    me.myPlayer = player
    me.guid = guid
...
def RequestGroups(pubsocket, guid):
    ...
    scoutUnits = []
    scoutNums = []
    #scoutUnits.append("Striker")
    scoutUnits.append(2)
    scoutNums.append(1)
    evgcom.PY_GROUP_InitGroup(pubsocket, guid, scoutUnits, scoutNums, "scoutTop")

    scoutBotUnits = []
    scoutBotNums = []
    #scoutBotUnits.append("Striker")
    scoutBotUnits.append(2)
    scoutBotNums.append(1)
    evgcom.PY_GROUP_InitGroup(pubsocket, guid, scoutBotUnits, scoutBotNums, "scoutBot")

    atkUnits = []
    atkNums = []
    #atkUnits.append("Controller")
    atkUnits.append(1)
    atkNums.append(22)
    evgcom.PY_GROUP_InitGroup(pubsocket, guid, atkUnits, atkNums, "attackers")

    tank0Units = []
    tank0Nums = []
    #tank0Units.append("Tank")
    tank0Units.append(0)
    tank0Nums.append(40)
    evgcom.PY_GROUP_InitGroup(pubsocket, guid, tank0Units, tank0Nums, "defenseGroup")
    #evgcom.PY_GROUP_InitGroup(pubsocket, guid, tank0Units, tank0Nums, "tanks")
    ...
    # tank1Units.append(0)
    # tank1Nums.append(20)
    # evgcom.PY_GROUP_InitGroup(pubsocket, guid, tank1Units, tank1Nums, "tanks2")

    elimUnits = []
    elimNums = []
    #elimUnits.append("Striker")
    elimUnits.append(2)
    elimNums.append(36)
    evgcom.PY_GROUP_InitGroup(pubsocket, guid, elimUnits, elimNums, "defenseGroup")
    #evgcom.PY_GROUP_InitGroup(pubsocket, guid, elimUnits, elimNums, "sweepers")

def FirstTurn(pubsocket):
    for group in gPlayers[me.myPlayer].groups:
        if group.tag == "tanks":
            print("splitting tank groups!TODO FIX")
            # numTanks = len(group.units[0].ids)
            # newGroupIds = []
            # for x in range(round(numTanks / 2)):
            #     newGroupIds.append(group.units[0].ids[x])
            # evgcom.PY_GROUP_CreateNewGroup(pubsocket, me.guid, group.location, newGroupIds, "tanks2")
        elif group.tag == "scoutTop":
            evgcom.PY_GROUP_MoveToNode(pubsocket, me.guid, group.groupID, me.atkPathTop[0])
            group.moving = True
            group.pathIndex = group.pathIndex + 1
        elif group.tag == "scoutBot":
            print("scout Node Bot: {0}    my location: {1}    pathIndex: {2}    atkPathLen: {3}".format(me.scoutNodeBot, group.location, group.pathIndex, len(me.atkPathBot)))
            evgcom.PY_GROUP_MoveToNode(pubsocket, me.guid, group.groupID, me.atkPathBot[0])
            group.moving = True
            group.pathIndex = group.pathIndex + 1

def update_sim(pubsocket, time_stamp):
    if me.firstTurn == True:
        me.firstTurn = False
        FirstTurn(pubsocket)
    else:
        for group in gPlayers[me.myPlayer].groups:
            if group.moving == False:
                if group.tag == "defenseGroup":
                    if me.homeBaseThreat == True:
                        if group.location != me.startNode:
                            print("send the defense home")
                            evgcom.PY_GROUP_MoveToNode(pubsocket, me.guid, group.groupID, me.startNode)
                            group.moving = True
                    else:
                        if me.topLaneThreat == True and group.location != me.defendNodeTop:
                            print("send the defense top")
                            evgcom.PY_GROUP_MoveToNode(pubsocket, me.guid, group.groupID, me.defendNodeTop)
                            group.moving = True
                        elif me.botLaneThreat == True and group.location != me.defendNodeBot:
                            print("send the defense bot")
                            evgcom.PY_GROUP_MoveToNode(pubsocket, me.guid, group.groupID, me.defendNodeBot)
                            group.moving = True

                # if group.tag == "tanks":
                #     if me.botLaneThreat == True and group.location != me.startNode:
                #         print("SEND THE TANKS0 HOME")
                #         evgcom.PY_GROUP_MoveToNode(pubsocket, me.guid, group.groupID, me.startNode)
                #         group.moving = True
                #     elif me.botLaneThreat == False and group.location != me.defendNodeTop:
                #         print("SEND THE TANKS0 TOP")
                #         evgcom.PY_GROUP_MoveToNode(pubsocket, me.guid, group.groupID, me.defendNodeTop)
                #         group.moving = True
                # elif group.tag == "tanks2":
                #     if me.topLaneThreat == True and group.location != me.startNode:
                #         print("SEND THE TANKS1 HOME")
                #         evgcom.PY_GROUP_MoveToNode(pubsocket, me.guid, group.groupID, me.startNode)
                #         group.moving = True
                #     elif me.topLaneThreat == False and group.location != me.defendNodeBot:
                #         print("SEND THE TANKS1 BOT")
                #         evgcom.PY_GROUP_MoveToNode(pubsocket, me.guid, group.groupID, me.defendNodeBot)
                #         group.moving = True
                # elif group.tag == "sweepers":
                #     if me.topLaneThreat == True and me.botLaneThreat == True:
                #         if group.location != me.startNode:
                #             print("SEND THE SWEEPERS HOME!")
                #             evgcom.PY_GROUP_MoveToNode(pubsocket, me.guid, group.groupID, me.startNode)
                #             group.moving = True
                #     elif me.topLaneThreat == True:
                #         if group.location != me.defendNodeTop:
                #             print("SEND THE SWEEPERS TOP!")
                #             evgcom.PY_GROUP_MoveToNode(pubsocket, me.guid, group.groupID, me.defendNodeTop)
                #             group.moving = True
                #     elif me.botLaneThreat == True:
                #         if group.location != me.defendNodeBot:
                #             print("SEND THE SWEEPERS BOT!")
                #             evgcom.PY_GROUP_MoveToNode(pubsocket, me.guid, group.groupID, me.defendNodeBot)
                #             group.moving = True
                elif group.tag == "attackers":
                    if me.topLaneThreat == True and group.location != me.targetNode:
                        print("SEND THE ATTACK BOTTOM! next node: {0}".format(me.atkPathBot[group.pathIndex]))
                        evgcom.PY_GROUP_MoveToNode(pubsocket, me.guid, group.groupID, me.atkPathBot[group.pathIndex])
                        group.moving = True
                        group.pathIndex = group.pathIndex + 1
                    elif me.botLaneThreat == True and group.location != me.targetNode:
                        print("SEND THE ATTACK TOP! next node: {0}".format(me.atkPathTop[group.pathIndex]))
                        evgcom.PY_GROUP_MoveToNode(pubsocket, me.guid, group.groupID, me.atkPathTop[group.pathIndex])
                        group.moving = True
                        group.pathIndex = group.pathIndex + 1
                elif group.tag == "scoutTop":
                    if group.location != me.scoutNodeTop:
                        print("SEND THE SCOUTS TOP")
                        evgcom.PY_GROUP_MoveToNode(pubsocket, me.guid, group.groupID, me.atkPathTop[group.pathIndex])
                        group.moving = True
                        group.pathIndex = group.pathIndex + 1
                elif group.tag == "scoutBot":
                    if group.location != me.scoutNodeBot:
                        print("SEND THE SCOUTS BOT")
                        print("scout Node Bot: {0}    my location: {1}    pathIndex: {2}    atkPathLen: {3}".format(me.scoutNodeBot, group.location, group.pathIndex, len(me.atkPathBot)))
                        evgcom.PY_GROUP_MoveToNode(pubsocket, me.guid, group.groupID, me.atkPathBot[group.pathIndex])
                        group.moving = True
                        group.pathIndex = group.pathIndex + 1
...
def check_message(record_type, json_data, pubsocket, time_stamp):
    if (evg.TurnStart.TypeId() == record_type):
        ...
        print("Combat update scouts!")
        msg = evg.PY_GROUP_CombatUpdate.from_message(json_data)
        for group in gPlayers[me.myPlayer].groups:
            if group.tag == "scoutTop":
                for id_ in msg.groups:
                    if id_ == group.groupID:
                        print("TOP LANE THREAT")
                        me.topLaneThreat = True
            if group.tag == "scoutBot":
                for id_ in msg.groups:
                    if id_ == group.groupID:
                        print("BOT LANE THREAT")
                        me.botLaneThreat = True
            print("I MADE IT OUT!")
        ...
        for unit in msg.types:
            count = msg.count[i]
            newUnit = evgUnit(unit)
            for x in range(count):
                newUnit.ids.append(msg.start[i] + x)
        ...
        #     newGroup.tag = "scouts"
        # elif groupId == 1:
        #     newGroup.tag = "attackers"
        # elif groupId == 2:
        #     newGroup.tag = "tanks"
        # elif groupId == 3:
        #     newGroup.tag = "sweepers"
        gPlayers[me.myPlayer].groups.append(newGroup)
        # if len(msg.types) < me.smallestGroupSize:
        #     me.smallestGroupSize = len(msg.types)
        #     me.strikeTeam = len(gPlayers[me.myPlayer].groups) - 1
    elif (evg.PY_GROUP_Knowledge.TypeId() == record_type):
        print("Group knowledge update!")
        msg = evg.PY_GROUP_Knowledge.from_message(json_data)
        if (msg.node1 == me.startNode or msg.node2 == me.startNode) and me.homeBaseThreat == False:
            print("HOME BASE THREAT group")
            me.homeBaseThreat = True
        elif (msg.node1 == 4 or msg.node1 == 7 or msg.node1 == 10) and me.botLaneThreat == False:
            print("BOT LANE THREAT group")
            me.botLaneThreat = True
        elif (msg.node1 == 2 or msg.node1 == 5 or msg.node1 == 8) and me.topLaneThreat == False:
            print("TOP LANE THREAT group")
            me.topLaneThreat = True
...
def program_main(): 
    ...
    evgcom.HelloWorld(pubsocket, guidstrP0, "scoutAndDefend")
    waiting = True
    while waiting:
        # Read envelope with address
        try:
